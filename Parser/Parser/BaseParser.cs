...
namespace AirdPro.Parser
{
    public class BaseParser
    {
        ...
        */
        ...
        public List<MsNResult> getMsN(int msN, double time)
        {
            var list = new List<MsNResult>();
            var msNBlock = airdInfo.indexList.Where(x => x.level == msN)
                .OrderBy(x => Math.Abs(x.rts.OrderBy(y => Math.Abs(y - time)).FirstOrDefault() - time))
                .FirstOrDefault();
            var blockValue = parseBlockValue(airdFile, msNBlock);
